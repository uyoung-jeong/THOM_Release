...
def render_sds(guidance_opt, embeddings, scene, gaussians, dataset, pipe, iteration, background, viewpoint_stack):
...
def training_mano(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)

    # gauassian model initialization
    ...
    if use_mano_mapping:
        if use_triplane:
            ...
            gaussians = HandGaussianWrapper(dataset.sh_degree, hand_gaussian_cfg)
    else:
        gaussians = HandGaussianModel(dataset.sh_degree, hand_gaussian_cfg)

    use_lap_loss = opt.lambda_lap > 0
    if use_lap_loss:
        LapReg_Loss = LaplacianReg(mano.vertex_num_upsampled, mano.face_upsampled)

    scene = HandScene(dataset, gcams, gaussians)

    gaussians.training_setup(opt, scene.mano_param_dict)
    if checkpoint:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        if opt.freeze_xyz>0 and iteration<=1:
            gaussians.mean_offset.requires_grad = False
            if learn_lbs_offset:
                gaussians.mean_offset_offset.requires_grad = False
            print(f'freeze xyz')
        if opt.freeze_xyz>0 and (opt.freeze_xyz+1 == iteration):
            gaussians.mean_offset.requires_grad = True
            if learn_lbs_offset:
                gaussians.mean_offset_offset.requires_grad = True
            print('thaw xyz')
        ...
        return_dict = render_sds(guidance_opt, embeddings, scene, gaussians, dataset, 
                                pipe, iteration, background, viewpoint_stack)
        viewpoint_stack = return_dict['viewpoint_stack']
        images = return_dict['images']
        text_z_ = return_dict['text_z_']
        weights_ = return_dict['weights_']
        alphas = return_dict['alphas']
        text_z_inverse = return_dict['text_z_inverse']
        depths = return_dict['depths']
        scales = return_dict['scales']
        render_pkg = return_dict['render_pkg']
        ...
        if guidance_opt.perpneg:
            loss_guide = guidance.train_step_perpneg(torch.stack(text_z_, dim=1), images, 
            ...
            loss_guide = guidance.train_step_rgb(torch.stack(text_z_, dim=1), images, 
                                    pred_depth=depths, pred_alpha=alphas,
                                    grad_scale=guidance_opt.lambda_guidance,
                                    use_control_net = use_control_net ,save_folder = save_folder,  iteration = iteration, warm_up_rate=warm_up_rate, 
            ...
            loss_guide = guidance.train_step(torch.stack(text_z_, dim=1), images, 
                                    pred_depth=depths, pred_alpha=alphas,
                                    grad_scale=guidance_opt.lambda_guidance,
                                    use_control_net = use_control_net ,save_folder = save_folder,  iteration = iteration, warm_up_rate=warm_up_rate, 
                                    resolution=(gcams.image_h, gcams.image_w),
        ...
        loss = loss_guide + opt.lambda_tv * loss_tv + \
            opt.lambda_scale * loss_scale  + opt.lambda_bin * loss_bin

        
        if use_lap_loss and use_mano_mapping:
        ...
        if tb_writer is not None:
            ...
            tb_writer.add_scalar('scale_refined/min', scaling.amin().item(), iteration)
            tb_writer.add_scalar('scale_refined/max', scaling.amax().item(), iteration)
            tb_writer.add_scalar('f_sh_refined/min', feats.amin().item(), iteration)
            tb_writer.add_scalar('f_sh_refined/max', feats.amax().item(), iteration)

            tb_writer.add_scalar('loss/guide', loss_guide.item(), iteration)
            tb_writer.add_scalar('loss/tv_scale', (opt.lambda_tv * loss_tv).item(), iteration)
            tb_writer.add_scalar('loss/scale', (opt.lambda_scale * loss_scale).item(), iteration)
            tb_writer.add_scalar('loss/bin', (opt.lambda_bin * loss_bin).item(), iteration)
            if use_lap_loss and use_mano_mapping:
                tb_writer.add_scalar('loss/lap', (loss_lap * opt.lambda_lap).item(), iteration)
            if opt.lambda_normal > 0:
                tb_writer.add_scalar('loss/normal', normal_loss.item(), iteration)
...
def training_report(tb_writer, iteration, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        if tb_writer:
            ...
            tb_writer.add_scalar('total_points', scene.gaussians._xyz.shape[0], iteration)
        torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    test_iter = [1, op.iterations//2, op.iterations]
    ...
    checkpoint_dir = training_mano(lp, op, pp, gcp, gp, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.save_video)

    end_time = datetime.now()

    # stage1 done
    print("hand training complete.\n")
