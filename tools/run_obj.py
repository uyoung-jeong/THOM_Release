...
from utils.loss_utils import tv_loss,NormalLoss, LaplacianReg
...
from utils.mesh_recon import obj_mesh_recon
...
def prepare_embeddings(guidance_opt, guidance):
    ...
    text = guidance_opt.prefix_text + guidance_opt.text + guidance_opt.postfix_text
...
def guidance_setup(guidance_opt):
    ...
    elif guidance_opt.guidance=="Zero123plus":
        from guidance.zero123p_utils import Zero123plus
        guidance = Zero123plus(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
                                   guidance_opt.t_range, guidance_opt.max_t_range, 
                                   num_train_timesteps=guidance_opt.num_train_timesteps, 
                                   ddim_inv=guidance_opt.ddim_inv,
                                   textual_inversion_path = guidance_opt.textual_inversion_path,
                                   LoRA_path = guidance_opt.LoRA_path,
                                   guidance_opt=guidance_opt)
    elif guidance_opt.guidance=="Zero123":
        from guidance.zero123_utils import Zero123
        guidance = Zero123(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
                                   guidance_opt.t_range, guidance_opt.max_t_range, 
                                   num_train_timesteps=guidance_opt.num_train_timesteps, 
                                   ddim_inv=guidance_opt.ddim_inv,
                                   textual_inversion_path = guidance_opt.textual_inversion_path,
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    gaussians = GaussianModel(dataset.sh_degree, gaussian_cfg)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        if opt.freeze_xyz>0 and iteration<=1:
            gaussians._xyz.requires_grad = False
            print(f'freeze xyz')
        if opt.freeze_xyz>0 and (opt.freeze_xyz+1 == iteration):
            gaussians._xyz.requires_grad = True
            print('thaw xyz')
        
        if opt.freeze_scale>0 and iteration<=1:
            gaussians._scaling.requires_grad = False
            print('freeze scaling')
        if opt.freeze_scale>0 and (opt.freeze_scale+1 == iteration):
            gaussians._scaling.requires_grad = True
            print('thaw scaling')
        # freeze xyz
        if opt.meshify_iter> 0 and iteration > opt.meshify_iter and not freeze_xyz:
            gaussians._xyz.requires_grad = False
            freeze_xyz = True

        iter_start.record()

        lr_it = iteration if iteration < opt.meshify_iter else max(iteration - opt.meshify_iter-1, 1)
        gaussians.update_learning_rate(lr_it)
        gaussians.update_feature_learning_rate(lr_it)
        gaussians.update_rotation_learning_rate(lr_it)
        gaussians.update_scaling_learning_rate(lr_it)
        ...
        elif iteration <= opt.rgb_opt_iter:
            loss = guidance.train_step_rgb(torch.stack(text_z_, dim=1), images, 
                                    pred_depth=depths, pred_alpha=alphas,
                                    grad_scale=guidance_opt.lambda_guidance,
                                    use_control_net = use_control_net ,save_folder = save_folder,  iteration = iteration, warm_up_rate=warm_up_rate, 
        ...
        loss = loss + opt.lambda_tv * loss_tv + opt.lambda_scale * loss_scale + opt.lambda_bin * loss_bin#opt.lambda_tv * loss_tv + opt.lambda_bin * loss_bin + opt.lambda_scale * loss_scale +

        # laplacian
        if LapReg_loss is not None:
        ...
        with torch.no_grad(): # Progress bar
            ...
            # Densification
            if not opt.no_adc and iteration < opt.meshify_iter:
                if iteration < opt.densify_until_iter and gaussians._xyz.shape[0]<opt.max_num_densify:
                ...
                if iteration % opt.opacity_reset_interval == 0: #or (dataset._white_background and iteration == opt.densify_from_iter)
                    gaussians.reset_opacity(min(opt.min_opacity, 0.5))
            elif iteration % 5 == 0:
                gaussians.clamp_opacity(opt.min_opacity)
            ...
            # mesh recon
            if iteration == opt.meshify_iter:
                # set opacity to be at least 0.5
                gaussians.clamp_opacity(opt.min_opacity)
                mesh_path = osp.join(dataset._model_path, f'meshify_{iteration}.ply')
                """
                meshify_method = opt.meshify_method
                if meshify_method == 'poisson':
                    depth = dataset.poisson_depth
                    ext_start_time = datetime.now()
                    #gaussians.meshify_poisson(depth, mesh_path, opt.max_num_densify, opt.min_opacity)
                    #gaussians.extract_poisson(depth, mesh_path, gcams.init_num_pts, opt.min_opacity)
                    extract_mesh_sugarstyle(opt, dataset, gaussians, mesh_path)
                    ext_end_time = datetime.now()
                    mesh_ext_time = ext_end_time - ext_start_time
                    print("mesh extraction time: ", mesh_ext_time)
                elif meshify_method == 'marching_cube':
                elif meshify_method == 'ball_pivot':
                    #radii = dataset.ball_pivot_radii
                    #gaussians.meshify_ball_pivot(radii, mesh_path, gcams.init_num_pts, opt.min_opacity)
                    raise NotImplementedError
                """
                obj_mesh_recon(opt, dataset, gaussians, iteration, mesh_path, opt_with_coarse=opt.opt_with_coarse)
                
                # instantiate laplacian loss
                if use_lap_loss:
                    n_verts = len(gaussians.verts)
                    faces = gaussians.faces.cpu().numpy()
                    LapReg_loss = LaplacianReg(n_verts, faces)

            # clamp opacity to have at least opt.min_opacity
            #opacities_new = torch.max(gaussians._opacity, torch.ones_like(gaussians._opacity)*opt.min_opacity)
            ...
            # set isotropic
            if opt.isotropic:
                scales_new = gaussians._scaling.mean(dim=1, keepdim=True).repeat(1,3).detach()
                gaussians._scaling = torch.nn.Parameter(scales_new, requires_grad=gaussians._scaling.requires_grad)
            """
    if opt.save_process:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_ratio", type=int, default=1) # [2500,5000,7500,10000,12000]
    ...
    parser.add_argument("--resume_ckpt", type=str, default = '', help='checkpoint dir to resume')
    parser.add_argument("--resume_stage", type=str, default = '', help='stage to resume. [sugar, refine]')
    ...
    #print('Stage1 OptimizationParams:')
    #print(f"feature_lr: {op.feature_lr}, lambda_scale: {op.lambda_scale}, min_opacity: {op.min_opacity}, warmup_iter: {op.warmup_iter}, isotropic: {op.isotropic}")
    #print(f"mano_param_lr: {op.mano_param_lr}, lambda_lap: {op.lambda_lap}")
    gp.text = args.prompt
    ...
    if args.resume_ckpt!='' and args.resume_stage!='':
        checkpoint_dir = args.resume_ckpt
    else:
        checkpoint_dir = training(lp, op, pp, gcp, gp, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.save_video)
