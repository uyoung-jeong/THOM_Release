...
def get_object_hand_info_thom(
    ...
    for text_idx in range(len(text_feat_clip)):
        ...
        is_lhand, is_rhand = search_hand(clip_model, mpnet, text_feat_clip_sp, text_feat_mpnet_sp)
        obj_path = object_model.obj_path
        verts_raw = object_model.verts_raw

        if isinstance(obj_path, list):
            obj_path = obj_path[text_idx]
        obj_mesh = trimesh.load(obj_path, maintain_order=True)
        obj_verts_org = proc_torch_cuda(obj_mesh.vertices.copy())
        point_set, obj_pc, obj_pc_normal, _ = object_model()
        ...
        obj_verts_list.append(obj_verts_org)
        obj_faces_list.append(obj_mesh.faces)
        verts_raw_list.append(verts_raw)
