...
def get_hand_joints_w_tip(hand_params, hand_layer):
    ...
    if out.joints.shape[1] == 21:
        joints_w_tip = [
                    out.joints[:,:16],
                    out.vertices[:,745].unsqueeze(1),
                    out.vertices[:,317].unsqueeze(1),
                    out.vertices[:,445].unsqueeze(1),
                    out.vertices[:,556].unsqueeze(1),
                    out.vertices[:,673].unsqueeze(1),
                    ]
        joints_w_tip = torch.cat(joints_w_tip, dim=1)
    else:
        joints_w_tip = out.joints_w_tip

    hand_joints_w_tip = joints_w_tip.reshape(bs, nframes, 21, 3)
