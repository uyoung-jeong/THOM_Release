...
class SuGaRDensifier():
    ...
    def reset_opacity(self, min_opacity=0.01):
        opacities_new = inverse_sigmoid(torch.min(self.model.strengths, torch.ones_like(self.model.all_densities.view(-1, 1))*min_opacity))
    ...
    def set_isotropic(self):
        print(f'self.model._scales.shape: {self.model._scales.shape}')
        scales_new = self.model._scales.mean(dim=1).repeat(N,3)
        optimizable_tensors = self.replace_tensor_to_optimizer(scales_new, "scales")
