...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, t_range=[0.02, 0.98], max_t_range=0.98, num_train_timesteps=None, 
        ...
        if is_safe_tensor:
            ...
            try:
                ...
                print(e)
                if model_key == 'stabilityai/stable-diffusion-2-1-base':
        ...
        #print(f'num_train_timesteps: {num_train_timesteps}, self.scheduler.config.num_train_timesteps: {self.scheduler.config.num_train_timesteps }') # None, 1000
    ...
    def train_step(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
        ...
        t = self.timesteps[ind_t]
        prev_t = self.timesteps[ind_prev_t]
        #print(f't: {t}, t.dtype: {t.dtype}') # torch.int64
        ...
        with torch.no_grad():
            ...
            unet_output = unet_output.reshape(2, -1, 4, resolution[0] // 8, resolution[1] // 8, ) # [2,4,4,64,64]
            noise_pred_uncond, noise_pred_text = unet_output[:1].reshape(-1, 4, resolution[0] // 8, resolution[1] // 8, ), unet_output[1:].reshape(-1, 4, resolution[0] // 8, resolution[1] // 8, ) #[4,4,64,64],[4,4,64,64]
            #print(f'unet_output.shape: {unet_output.shape}, noise_pred_uncond.shape: {noise_pred_uncond.shape}, noise_pred_text.shape: {noise_pred_text.shape}, target.shape: {target.shape}')
        ...
        w = lambda alphas: (((1 - alphas) / alphas) ** 0.5)     
        #print(f't: {t}') # 959
        #print(f'self.alphas[t].dtype: {self.alphas[t].dtype} pred_noise.dtype: {pred_noise.dtype}, target.dtype: {target.dtype}') #  # f32, f16, f16
        ...
        if iteration % guidance_opt.vis_interval == 0:
            ...
            with torch.no_grad():
                ...
                latents_sp_rgb = F.interpolate(lat2rgb(pred_x0_latent_sp), (resolution[0], resolution[1]), mode='bilinear', align_corners=False)
                """
                print(f'as_latent: {as_latent}, self.noise_temp.shape: {self.noise_temp.shape}') # False, [4,4,64,64,]
                print(f'text_embeddings.shape: {text_embeddings.shape}, inverse_text_embeddings.shape: {inverse_text_embeddings.shape}') # [8, 77, 1024], [8, 77, 1024]
                print(f'guidance_opt.delta_t: {guidance_opt.delta_t}, current_delta_t: {current_delta_t}') # 80, 93
                print(f't: {t}, prev_t: {prev_t}')# 673, 580
                print(f'self.ism: {self.ism}') # True
                print(f'prev_latents_noisy.shape: {prev_latents_noisy.shape}') # [4, 4, 64, 64]
                print(f'latents_noisy.shape: {latents_noisy.shape}') # [4, 4, 64, 64]
                print(f'use_control_net: {use_control_net}') # False
                print(f'latent_model_input.shape: {latent_model_input.shape}') # [8, 4, 64, 64]
                print(f'self.precision_t: {self.precision_t}, tt.shape: {tt.shape}') # torch.float16, [8]
                print(f'unet_output.shape: {unet_output.shape}') # [2, 4, 4, 64, 64]
                """
    ...
    def train_step_rgb(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
                    grad_scale=1,use_control_net=False,
                    save_folder:Path=None, iteration=0, warm_up_rate = 0,
                    resolution=(512, 512), guidance_opt=None,as_latent=False, embedding_inverse = None):
        ...
        with torch.no_grad():
            ...
            pred_x0_latent_pos = pred_original(self.scheduler, noise_pred_post, prev_t, prev_latents_noisy)
            pred_x0_pos = self.decode_latents(pred_x0_latent_pos.type(self.precision_t))
            rgb_mask = pred_depth > 0
            n_mask = rgb_mask.sum()

        if n_mask.item() > 0:
            rgb_loss = torch.sum(torch.abs(pred_rgb - pred_x0_pos) * rgb_mask)/n_mask
        else:
            rgb_loss = .0
        loss = loss + rgb_loss
