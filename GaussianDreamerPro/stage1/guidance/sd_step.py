...
def ddim_step(
    ...
    **kwargs
    ...
    #print(f'self.config.thresholding: {self.config.thresholding}') # False
    if hasattr(self.config, 'thresholding'):
    ...
    if hasattr(self, '_get_variance'):
        variance = abs(self._get_variance(timestep, prev_timestep))
    else: # https://github.com/huggingface/diffusers/blob/v0.34.0/src/diffusers/schedulers/scheduling_ddim.py
        alpha_prod_t = self.alphas_cumprod[timestep]
        alpha_prod_t_prev = self.alphas_cumprod[prev_timestep] if prev_timestep >= 0 else self.final_alpha_cumprod
        beta_prod_t = 1 - alpha_prod_t
