...
class HandGaussianModel:
    ...
    def __init__(self, sh_degree, cfg):
        ...
        #self.shape_param = nn.Parameter(torch.zeros((), dtype=torch.float32, device='cuda'))
        self.hand_rescaling = cfg.hand_rescaling
        self.hand_trans = torch.zeros((1, 3), dtype=torch.float32, device="cuda")
    ...
    def create_from_mano(self, mano_params, shape_param, spatial_lr_scale):
        ...
        vertices_upsampled = vertices_upsampled * self.hand_rescaling

        fused_point_cloud = vertices_upsampled
        #colors = torch.zeros((vertices_upsampled.shape[0], 3), dtype=torch.float32).cuda() + 32.0
        #colors = torch.rand((vertices_upsampled.shape[0], 3), dtype=torch.float32).cuda()
        # value is obtained from GaussianDreamerPro iniitialization values
        ...
        fused_color = RGB2SH(colors)
        fused_color[:,0] = 0.05532715
        fused_color[:,1] = -0.04348575
        fused_color[:,2] = -0.01990034
    ...
    def save_ply(self, path):
        ...
        PlyData([el, shape_el]).write(path)
...
class MANOParamDict(nn.Module):
    def __init__(self):
        super(MANOParamDict, self).__init__()

    # initialize MANO parameters at single frame
    def init(self, mano_params):
        param_dict = nn.ParameterDict({})

        root_param = mano_params['root_pose'].cuda()
        hand_param = mano_params['hand_pose'].cuda()
        trans_param = mano_params['trans'].cuda()
        
        param_dict['root_pose'] = nn.Parameter(root_param) # [1, 6]
        param_dict['hand_pose'] = nn.Parameter(hand_param) # [15, 6]
        param_dict['trans'] = nn.Parameter(trans_param) # [1, ]
        
        self.mano_params = nn.ParameterDict(param_dict)

    def get_optimizable_params(self, mano_param_lr):
        optimizable_params = []
        for param_name in self.mano_params.keys():
            optimizable_params.append({'params': [self.mano_params[param_name]], 
                                        'name': 'mano_' + param_name, 
                                        'lr': mano_param_lr})
    ...
    def forward(self):
        ...
        for param_name in self.mano_params.keys():
            if 'pose' in param_name:
                mano_param[param_name] = matrix_to_axis_angle(rotation_6d_to_matrix(self.mano_params[param_name]))
            else:
                mano_param[param_name] = self.mano_params[param_name]
        out.append(mano_param)
        return out
