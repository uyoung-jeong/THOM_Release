...
from utils.mano_utils import get_mano_param

class HandScene:

    gaussians : HandGaussianModel

    def __init__(self, args : ModelParams, pose_args : GenerateCamParams, gaussians : HandGaussianModel, load_iteration=None, shuffle=False, resolution_scales=[1.0]):
        """b
        ...
        # mano param initialization
        device = 'cuda:0'

        # read 
        self.mano_param_dict = get_mano_param(args.mano_path)
        shape_param = torch.zeros((1, 10), device=device, dtype=torch.float32)
        ...
        self.use_triplane = args.use_triplane
        if self.loaded_iter:
            if self.use_triplane:
                self.gaussians.load_pth(os.path.join(self.model_path,
                                                           "point_cloud",
            ...
            if self.use_triplane:
                self.gaussians.load_pth(self.pretrained_model_path)
            else:
                print(f'loading pretraind hand ply file from {self.pretrained_model_path}')
                self.gaussians.load_ply(self.pretrained_model_path)
        else:
            self.gaussians.create_from_mano(self.mano_param_dict()[0], shape_param, self.cameras_extent)

    def save(self, iteration):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        if self.use_triplane:
            self.gaussians.save_pth(os.path.join(point_cloud_path, "point_cloud.pth"))
        else:
