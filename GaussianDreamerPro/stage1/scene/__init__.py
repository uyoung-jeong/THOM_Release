...
from scene.dataset_readers import sceneLoadTypeCallbacks,GenerateRandomCameras,GeneratePurnCameras,GenerateCircleCameras,GenerateOutCameras,GenerateSoftPruneCameras
...
from scene.hand_gaussian_model import HandGaussianModel
from scene.hand_gaussian_wrapper import HandGaussianWrapper
from scene.hand_gaussian_triplane_wrapper import HandGaussianTriplaneWrapper
from scene.hand_scene import HandScene
from scene.hoi_gaussian_model import HOIGaussianModel
from scene.hoi_scene import HOIScene
...
class Scene:
    ...
    def __init__(self, args : ModelParams, pose_args : GenerateCamParams, gaussians : GaussianModel, load_iteration=None, shuffle=False, resolution_scales=[1.0]):
        ...
        if self.loaded_iter:
            ...
            if self.pretrained_model_path.endswith('.ply'):
                print(f'loading pretraind object ply file from {self.pretrained_model_path}')
                self.gaussians.load_ply(self.pretrained_model_path)
        else:
            #init_lgm_path = os.path.join(self.model_path, "init_lgm.ply")
            #self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent, init_lgm_path=init_lgm_path)
    ...
    def getSoftPruneCameras(self, scale=1.0,batch_size=12, render45 = True):
        video_circle_cameras = GenerateSoftPruneCameras(self.pose_args, batch_size)
        video_cameras = {}
        for resolution_scale in self.resolution_scales:
            video_cameras[resolution_scale] = cameraList_from_RcamInfos(video_circle_cameras, resolution_scale, self.pose_args)        
        return video_cameras[scale]